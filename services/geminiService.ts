...
export const geminiService = {
  ...
  async generateScoringInsight(
    ...
    const result = await callServerAPI('generateScoringInsight', { decision, scores, rankings });
  ...
  async brainstormRisks(
    ...
    const result = await callServerAPI('brainstormRisks', { decision, topChoice });
    return result?.risks || null;
  },
  async refineMemo(rawText: string): Promise<string> {
    const result = await callServerAPI('refineMemo', { rawText });
