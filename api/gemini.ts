...
Context: ${decision.objectives || ''}
Current alternatives: ${(decision.alternatives || []).map((a: any) => a.name).join(', ')}
...
{"suggestions": ["alt1", "alt2", "alt3"], "rationale": "brief explanation"}`;
      ...
      prompt = `You are a decision analyst. The top-ranked option is "${top}" for this decision: "${decision.statement}".
...
{"insight": "your insight here"}`;
      ...
      prompt = `You are a risk analyst. For this decision: "${decision.statement}", top choice: "${topChoice.name}".
